...
for ii = 1:iter
    ...
    for snr_iter = 1:length(SNRdB)
        ...
        X_DD_u2(9:16,5:29) = sqrt((1/2)*SNR(snr_iter))*((2*randi([0,1],8,25)-1)+1i*(2*randi([0,1],8,25)-1));
        X_DD_u3(17:25,1:32) = sqrt((1/2)*SNR(snr_iter))*((2*randi([0,1],9,32)-1)+1i*(2*randi([0,1],9,32)-1));
        X_DD_u3(26:32,9:29) = sqrt((1/2)*SNR(snr_iter))*((2*randi([0,1],7,21)-1)+1i*(2*randi([0,1],7,21)-1));
