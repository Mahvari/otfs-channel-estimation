...
for ii = 1:iter
    ...
    for snr_iter = 1:length(SNRdB)
        ...
        X_DD_u1(9:25,:) = sqrt((1/2)*SNR(snr_iter))*((2*randi([0,1],17,32)-1) + 1i*(2*randi([0,1],17,32)-1));
        X_DD_u1(1:8,5:29) = sqrt((1/2)*SNR(snr_iter))*((2*randi([0,1],8,25)-1) + 1i*(2*randi([0,1],8,25)-1));
        X_DD_u1(26:32,5:29) = sqrt((1/2)*SNR(snr_iter))*((2*randi([0,1],7,25)-1) + 1i*(2*randi([0,1],7,25)-1));
        ...
        BER_MMSE_ICSI_LMMSE(snr_iter) = BER_MMSE_ICSI_LMMSE(snr_iter) + sum(DecodedSyms_LMMSE_real ~= real(x_DD_S))+ sum(DecodedSyms_LMMSE_imag ~= imag(x_DD_S));
        ...
        DecodedSyms_LMMSE_real = sqrt((1/2)*SNR(snr_iter))*(2*DecodedBits_LMMSE_real - 1);
        DecodedSyms_LMMSE_imag = sqrt((1/2)*SNR(snr_iter))*(2*DecodedBits_LMMSE_imag - 1);
        BER_MMSE_PCSI_LMMSE(snr_iter) = BER_MMSE_PCSI_LMMSE(snr_iter) + sum(DecodedSyms_LMMSE_real ~= real(x_DD_S))+ sum(DecodedSyms_LMMSE_imag ~= imag(x_DD_S));
