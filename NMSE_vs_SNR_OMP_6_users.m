...
pilot_vector_1 = 2*randi([0,1],160,1)-1;      %bpsk pilot symbols for user 1
pilot_vector_2 = 2*randi([0,1],160,1)-1;      %bpsk pilot symbols for user 2
pilot_vector_3 = 2*randi([0,1],160,1)-1;      %bpsk pilot symbols for user 3
pilot_vector_4 = 2*randi([0,1],160,1)-1;      %bpsk pilot symbols for user 4
pilot_vector_5 = 2*randi([0,1],160,1)-1;      %bpsk pilot symbols for user 5
pilot_vector_6 = 2*randi([0,1],160,1)-1;      %bpsk pilot symbols for user 6


% Pilot Tx vectors
pilot_vector_1 = [pilot_vector_1;zeros(864,1)];   
pilot_vector_2 = [zeros(160,1);pilot_vector_2;zeros(704,1)];   
pilot_vector_3 = [zeros(320,1);pilot_vector_3;zeros(544,1)];    
pilot_vector_4 = [zeros(480,1);pilot_vector_4;zeros(384,1)];                 
pilot_vector_5 = [zeros(640,1);pilot_vector_5;zeros(224,1)];                 
pilot_vector_6 = [zeros(800,1);pilot_vector_6;zeros(64,1)];                
...
sensing_matrix_5 = sensing_matrix_creation(M,N,l_max,k_max,n_pilots,pilot_vector_5);
sensing_matrix_6 = sensing_matrix_creation(M,N,l_max,k_max,n_pilots,pilot_vector_6);

sensing_matrix = [sensing_matrix_1 sensing_matrix_2 sensing_matrix_3 sensing_matrix_4 sensing_matrix_5 sensing_matrix_6];
...
for ii = 1:iter
    ...
    delay_taps_5 = randperm(l_max,taps)-1;
    delay_taps_6 = randperm(l_max,taps)-1;
    ...
    doppler_taps_5 = randperm(k_max,taps)-1;
    doppler_taps_6 = randperm(k_max,taps)-1;
    ...
    chann_coeff_5 = (1/sqrt(2))*(randn(1,taps) + 1j*randn(1,taps));
    chann_coeff_6 = (1/sqrt(2))*(randn(1,taps) + 1j*randn(1,taps));

    % noise factor
    noise = (sqrt(1/2))*(randn(1,n_pilots) + 1j*randn(1,n_pilots));
    
    for k = 1:length(SNR_dB)
        disp("# of iteraction: " + ii + "; SNR: " + SNR_dB(k))
        ...
        TxSamples_5 = sqrt(SNR(k))*pilot_vector_5.';
        TxSamples_6 = sqrt(SNR(k))*pilot_vector_6.';
        ...
        TxSamplesCP_5 = [TxSamples_5(n_pilots-Ncp+1:end) TxSamples_5];
        TxSamplesCP_6 = [TxSamples_6(n_pilots-Ncp+1:end) TxSamples_6];
        ...
        RxsamplesCP_5 = zeros(1,n_pilots+Ncp)+1j*zeros(1,n_pilots+Ncp);
        RxsamplesCP_6 = zeros(1,n_pilots+Ncp)+1j*zeros(1,n_pilots+Ncp);
        ...
        for tap_iter = 1:taps
           ...
           RxsamplesCP_5 = RxsamplesCP_5 + chann_coeff_5(tap_iter)*circshift(TxSamplesCP_5.*exp(1j*2*pi/M*...
               (-Ncp:n_pilots-1)*doppler_taps_5(tap_iter)/N),[0,delay_taps_5(tap_iter)]);
           RxsamplesCP_6 = RxsamplesCP_6 + chann_coeff_6(tap_iter)*circshift(TxSamplesCP_6.*exp(1j*2*pi/M*...
               (-Ncp:n_pilots-1)*doppler_taps_6(tap_iter)/N),[0,delay_taps_6(tap_iter)]);
        end
        ...
        Rxsamples_5 = (RxsamplesCP_5(Ncp+1:n_pilots+Ncp));
        Rxsamples_6 = (RxsamplesCP_6(Ncp+1:n_pilots+Ncp));

        % Total Tx signal
        Rxsamples = (Rxsamples_1 + Rxsamples_2 + Rxsamples_3 + Rxsamples_4 + Rxsamples_5 + Rxsamples_6 + noise).'; 
        ...
        h_sparse_omp = OMP_estimation(Rxsamples,sensing_matrix_snr,3);
        ...
        h_sparse_actual_5 = h_sparse_creation(M,N,l_max,k_max,n_pilots,chann_coeff_5,taps,delay_taps_5,doppler_taps_5);
        h_sparse_actual_6 = h_sparse_creation(M,N,l_max,k_max,n_pilots,chann_coeff_6,taps,delay_taps_6,doppler_taps_6);
        h_sparse_actual = [h_sparse_actual_1;h_sparse_actual_2;h_sparse_actual_3;h_sparse_actual_4; h_sparse_actual_5; h_sparse_actual_6];
